...
import org.example.autoreview.domain.comment.dto.response.CommentResponseDto;
...
public class CommentService {
    ...
    public Long save(CommentSaveRequestDto requestDto, String email) {
        ...
        if (requestDto.parentId() != null) {
            Comment parent = commentCommand.findById(requestDto.parentId());
    ...
    // 상위 댓글 조회
    public CommentListResponseDto findByCommentPage(Pageable pageable) {
        Page<Comment> commentPage = commentRepository.findByCommentPage(pageable);
    ...
    // 하위 댓글 조회
    public CommentListResponseDto findByReplyPage(Long parentId, Pageable pageable) {
        Page<Comment> commentPage = commentRepository.findByReplyPage(parentId, pageable);
        List<CommentResponseDto> dtoList = convertPageToListDto(commentPage);

        return new CommentListResponseDto(dtoList, commentPage.getTotalPages());
    }

    private List<CommentResponseDto> convertPageToListDto(Page<Comment> page) {
        return page.stream()
    ...
    public Long update(CommentUpdateRequestDto requestDto, String email) {
        Comment comment = commentCommand.findById(requestDto.commentId());
